...
# 		self.re_pattern_2_rever_comp = re.compile(self.get_expansion_bases(pattern_1, True))

	def get_expansion_bases(self, seq, b_reverse_complemente = False):
		sz_return = ''
		for base in seq:
			if (base in self.dict):
				if (b_reverse_complemente):	sz_return += utils.complement(self.dict[base])
...
if __name__ == '__main__':
	...
	if (len(sys.argv) != 5):
		print('Usage: {} <pattern_1> <pattern_2> <in> <out>'.format(os.path.basename(sys.argv[0])))
		sys.exit(0)
	
	pattern_1 = sys.argv[1]
	pattern_2 = sys.argv[2]
	file_in = sys.argv[3]
	file_out = sys.argv[4]
	...
	get_seq = GetSeq(pattern_1, pattern_2)
	dict_result = {}
	n_count_seq = 0
